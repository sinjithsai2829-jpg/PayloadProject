...
async function restoreSnapshot(snapshot) {
  ...
  // Compare from Code view first. Tree view depends on parsed data generated by
  // the comparison/format pipeline and can be restored safely afterwards.
  for (let index = 0; index < 2; index += 1) {
  ...
  // Give Tree rendering and synchronized view mirroring time to settle before
  // putting the saved viewport back in place.
  await frame();
  await frame();
